...
describe('BlankHoodie contract', () => {
  describe('mint() functionality', () => {
    ...
    it('user should be able to mint with enough ether', async () => {
      ...
      await hhBlankHoodie.connect(user).mint(1, { value: ethers.utils.parseEther('0.1') });
    ...
    it('should revert if user tries to mint without enough ether', async () => {
      // tx should revert
      await expect(hhBlankHoodie.connect(user).mint(1, { value: ethers.utils.parseEther('0.08') }))
        .to.be.reverted;
    ...
    it('should revert if max supply is exceeded', async () => {
      ...
      await expect(hhBlankHoodie.connect(user).mint(1, { value: ethers.utils.parseEther('0.1') }))
        .to.be.reverted;
    });
  });
  ...
  describe('airDrop() functionality', () => {
    ...
    it('should revert if caller not the owner', async () => {
      ...
      await expect(hhBlankHoodie.connect(user).airDrop(owner.address, 2)).to.be.revertedWith(
  ...
  describe('setBaseURI() functionality', () => {
    ...
    it('should revert if caller not the owner', async () => {
      expect(hhBlankHoodie.connect(user).setBaseURI('some-new-URI')).to.be.revertedWith(
        'Ownable: Caller not the owner'
      );
    });
  });
  ...
  describe('setPaused() functionality', () => {
    ...
    it('pause/unpause minting', async () => {
      ...
      await expect(hhBlankHoodie.mint(2, { value: ethers.utils.parseEther('0.2') })).to.be.reverted;

      // unpause
      await hhBlankHoodie.setPaused(false);
      // minting should work
      await expect(hhBlankHoodie.mint(2, { value: ethers.utils.parseEther('0.2') })).to.not.be
        .reverted;
    });

    it('should revert if caller not the owner', async () => {
      await expect(hhBlankHoodie.connect(user).setPaused(true)).to.be.revertedWith(
        'Ownable: caller is not the owner'
      );
    });
  });
  ...
  describe('setCost() functionality', () => {
    ...
    it('should change cost accordingly if called by owner', async () => {
      ...
      await expect(hhBlankHoodie.connect(user).mint(1, { value: ethers.utils.parseEther('1') })).to
        .not.be.reverted;
    });
  ...
  describe('withdraw() functionality', () => {
    ...
    it('should transfer contract balance to owner if called by owner', async () => {
      ...
      await hhBlankHoodie.connect(user).mint(5, { value: ethers.utils.parseEther('0.5') });
